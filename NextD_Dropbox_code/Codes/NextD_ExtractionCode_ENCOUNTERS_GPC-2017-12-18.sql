...
2. External to PCORNET table CAP_ENCOUNTER with encounters (from whole data warehouse) mapped to PCORNET types.
3. External to PCORNET table (#RawInsuranceValuesTable) with insurance values on encounters of interest.
4. External to PCORNET table (#RawNPIvaluesTable) with NPI values on encounters of interest. 
...
--------------------------------------------------------------------------------------------------------------- 
select i.PATID,i.ENCOUNTERID,i.PROVIDERID, i.ADMIT_DATE_YEAR,i.ADMIT_DATE_MONTH,i.DAYS_from_FirstEncounter_Date,i.ENC_TYPE,i.FACILITYID,i.IndividualProviderCategories,
/* omit this variable if not remapping insurance*/i.PrimaryInsuranceType  
into #NextD_ENCOUNTER_FINAL
from (select c.PATID,a.ENCOUNTERID,a.PROVIDERID,year(a.ADMIT_DATE) as ADMIT_DATE_YEAR,month(a.ADMIT_DATE) as ADMIT_DATE_MONTH,datediff(d,c.[FirstVisit],a.ADMIT_DATE) as DAYS_from_FirstEncounter_Date,a.ENC_TYPE,a.FACILITYID,
			...
			when b.NPI is not NULL and b.NPI in ('363L00000X','363LA2100X','363LA2200X','363LC0200X','363LC1500X','363LF0000X','363LG0600X','363LN0000X','363LN0005X','363LP0200X','363LP0222X',
			...
			/*omit this line if not remapping insurance*/left join /* provide name of external source table with raw encounter types and NPI codes here: */ #NEXTD_ENCOUNTERSRemap b on a.[ENCOUNTERID]=b.[ENCOUNTERID]
