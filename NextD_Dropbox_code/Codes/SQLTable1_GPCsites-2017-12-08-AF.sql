...
select e.CAP_ID, e.ADMIT_DATE, row_number() over (partition by e.PATID order by e.ADMIT_DATE asc) rn 
...
select x.CAP_ID, count(distinct x.ADMIT_DATE) as NumberOfPermutations 
into #Denomtemp1 
from #Denomtemp0 x group by x.CAP_ID order by x.CAP_ID;
-- Collect date of the first visit:
select x.CAP_ID, x.ADMIT_DATE as FirstVisit 
into #Denomtemp2 
...
select dia.PATID, dia.ADMIT_DATE 
into #Miscarr_Abort
	from capricorn.dbo.DIAGNOSIS as dia join #Denomtemp2 w on dia.PATID=w.CAP_ID
	join capricorn.dbo.ENCOUNTER e on dia.ENCOUNTERID=e.ENCOUNTERID 
	join capricorn.dbo.DEMOGRAPHIC d on e.PATID=d.PATID
	where ((dia.DX like '63[0-9]._%' or dia.DX like '6[4-7][0-9]._%' or dia.DX like 'V2[2-3].%' or dia.DX like 'V28.%') and dia.DX_TYPE = '09') or 
		...
		and dia.ADMIT_DATE >= @LowerTimeFrame and dia.ADMIT_DATE <= @UpperTimeFrame;
-- Cases with delivery procedures in ICD-9 coding:
select p.PATID, p.ADMIT_DATE 
into #DelivProc 
from capricorn.dbo.PROCEDURES as p 
	join #Denomtemp2 w on p.PATID=w.CAP_ID
	join capricorn.dbo.ENCOUNTER e on p.ENCOUNTERID=e.ENCOUNTERID 
	join capricorn.dbo.DEMOGRAPHIC d on e.PATID=d.PATID	
	where (p.PX like '7[2-5]._%' and p.PX_TYPE = '09') or (p.PX like '10%' and p.PX_TYPE = '10') or (p.PX like '59[0-9][0-9][0-9]' and p.PX_TYPE='CH') 
		...
		and p.ADMIT_DATE >= @LowerTimeFrame and p.ADMIT_DATE <= @UpperTimeFrame;
---------------------------------------------------------------------------------------------------------------
...
---------------------------------------------------------------------------------------------------------------
---------------------------------------------------------------------------------------------------------------
select a.CAP_ID as PATID, b.FirstVisit, a.NumberOfPermutations as NumerOfVisits,/* x.EventDate as DMonsetDate,*/ d.DEATH_DATE, 
...
from
	...
	from 
		(select l.PATID, l.LAB_ORDER_DATE,p.[1],p.[2],p.[3],p.[4],p.[5],p.[6],p.[7],p.[8],p.[9],p.[10]
		from capricorn.dbo.LAB_RESULT_CM l join #Denomtemp2 w on l.PATID=w.CAP_ID
		join capricorn.dbo.ENCOUNTER e on l.ENCOUNTERID=e.ENCOUNTERID 
		join capricorn.dbo.DEMOGRAPHIC d on e.PATID=d.PATID
		left join #FinalPregnancy p on l.PATID=p.PATID
		where
			(l.LAB_NAME='A1C' or l.LAB_LOINC in ('17855-8', '4548-4','4549-2','17856-6','41995-2','59261-8','62388-4','71875-9','54039-3'))
			and l.RESULT_NUM >=6.5 and l.RESULT_UNIT='PERCENT' 
			and e.ENC_TYPE in ('IP','ED','OS','AV','IS')
			and (datediff(yy,d.BIRTH_DATE,l.LAB_ORDER_DATE) <= @UpperAge and datediff(yy, d.BIRTH_DATE,l.LAB_ORDER_DATE) >= @LowerAge) 
			and l.LAB_ORDER_DATE >= @LowerTimeFrame and l.LAB_ORDER_DATE <= @UpperTimeFrame
		) i
	) i2
...
from
	...
	case when i.[10] is not NULL and ABS(datediff(yy,i.LAB_ORDER_DATE,i.[10]))>1 then 0 when i.[10] is not NULL and ABS(datediff(yy,i.LAB_ORDER_DATE,i.[10]))<=1 then 1 when i.[10] is NULL then 0 end as C10
	from 
		(select l.CAP_ID as PATID, l.LAB_ORDER_DATE,p.[1],p.[2],p.[3],p.[4],p.[5],p.[6],p.[7],p.[8],p.[9],p.[10]
		from capricorn.dbo.CAP_LABS l join #Denomtemp2 w on l.CAP_ID=w.CAP_ID
		join capricorn.dbo.CAP_ENCOUNTERS e on l.ENCOUNTERID=e.ENCOUNTERID 
		join capricorn.dbo.CAP_DEMOGRAPHICS d on e.CAP_ID=d.CAP_ID
		left join #FinalPregnancy p on l.CAP_ID=p.PATID
		where
			l.LAB_LOINC in ('1558-6',  '10450-5', '1554-5', '17865-7','35184-1') 
			and l.VALUE_NUMERIC >= 126 and l.UNIT='mg/dL' and RESULT_NUMERIC='Y' and e.ENC_TYPE in ('IP','ED','OS','AV','IS')
			and datediff(yy,d.BIRTH_DATE,l.LAB_ORDER_DATE) <=@UpperAge and datediff(yy,d.BIRTH_DATE,l.LAB_ORDER_DATE) >=@LowerAge
			and l.LAB_ORDER_DATE >= @LowerTimeFrame and l.LAB_ORDER_DATE <= @UpperTimeFrame
		) i
	) i2
...
from
	...
	from (
		...
		where
			...
			and l.VALUE_NUMERIC >= 200 and l.UNIT='mg/dL' and RESULT_NUMERIC='Y') and e.ENC_TYPE in ('IP','ED','OS','AV','IS')
			and datediff(yy, d.BIRTH_DATE,l.LAB_ORDER_DATE) <=@UpperAge and datediff(yy, d.BIRTH_DATE,l.LAB_ORDER_DATE) >=@LowerAge
			and l.LAB_ORDER_DATE >= @LowerTimeFrame and l.LAB_ORDER_DATE <= @UpperTimeFrame
		) i
	) i2
...
-----     uncoment line below if not interested in the code performace evaluation     -----
/*SET @Time13 = GETDATE();*/
--select count(*) from #A1cRG_final_FirstPair;
---------------------------------------------------------------------------------------------------------------
...
from
	...
	from (
		select e.PATID, l.ADMIT_DATE,p.[1],p.[2],p.[3],p.[4],p.[5],p.[6],p.[7],p.[8],p.[9],p.[10]
		from capricorn.dbo.DIAGNOSIS l join #Denomtemp2 w on l.PATID=w.CAP_ID
		join capricorn.dbo.ENCOUNTER e on l.ENCOUNTERID=e.ENCOUNTERID 
		join capricorn.dbo.DEMOGRAPHIC d on e.PATID=d.PATID
		left join #FinalPregnancy p on l.PATID=p.PATID
		where
			...
			and l.ENC_TYPE in ('IP','ED','OS','AV','IS') and datediff(yy, d.BIRTH_DATE, l.ADMIT_DATE) <=@UpperAge and datediff(yy, d.BIRTH_DATE, l.ADMIT_DATE) >=@LowerAge
			and l.ADMIT_DATE >= @LowerTimeFrame and l.ADMIT_DATE <= @UpperTimeFrame
		) i
	) i2
where i2.C1=0 and i2.C1=0 and i2.C2=0 and i2.C3=0 and i2.C4=0 and i2.C5=0 and i2.C6=0 and i2.C7=0 and i2.C8=0 and i2.C9=0 and i2.C10=0;	
...
from capricorn.dbo.PRESCRIBING a join #Denomtemp2 w on a.PATID=w.CAP_ID
join capricorn.dbo.ENCOUNTER e on a.ENCOUNTERID=e.ENCOUNTERID
join capricorn.dbo.DEMOGRAPHIC d on e.PATID=d.PATID
where (
...
and a.RX_ORDER_DATE >= @LowerTimeFrame and a.RX_ORDER_DATE <= @UpperTimeFrame;
...
and a.RX_ORDER_DATE >= @LowerTimeFrame and a.RX_ORDER_DATE <= @UpperTimeFrame;
/*SET @Time19 = GETDATE();*/
select count(*) from #AlphaGlucosidaseInhByByRXNORM_initial;
...
from #InclusionUnderRestrictionMeds_initial x join #A1c_final_FirstPair y on x.PATID=y.PATID
...
from #InclusionUnderRestrictionMeds_initial x join #FG_final_FirstPair y on x.PATID=y.PATID
where abs(datediff(dd,x.MedDate,y.EventDate))>1;
select count(*) from #p3;
-- Get set of patients having one med & random glucose measurement:
select x.PATID, x.MedDate  into #p4
from #InclusionUnderRestrictionMeds_initial x join #RG_final_FirstPair y on x.PATID=y.PATID
where abs(datediff(dd,x.MedDate,y.EventDate))>1;
...
---------------------------------------------------------------------------------------------------------------
---------------------------------------------------------------------------------------------------------------
select a.*, x.EventDate as DMonsetDate
into #FinalStatTable1
from #FinalStatTable01 a left join #All x on a.PATID=x.PATID;
---------------------------------------------------------------------------------------------------------------
---------------------------------------------------------------------------------------------------------------
